...
image_versions=$(docker-tags ${tag_image} | jq --raw-output '.["tags"] | to_entries | .[] | .value' | grep "${calibre_version}-")
echo "image_versions: $image_versions"

calibre_version_major=$(echo ${calibre_version} | sed -E "s/([0-9]*)\.[0-9]*\.[0-9]*/\1/")
calibre_version_minor=$(echo ${calibre_version} | sed -E "s/([0-9]*\.[0-9]*)\.[0-9]*/\1/")

tag_versions="${calibre_version} ${calibre_version_major} ${calibre_version_minor}"
